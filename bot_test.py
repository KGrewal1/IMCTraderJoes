    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.position.keys():
            ...
            if product in ['PINA_COLADAS', 'COCONUTS']:
                net_position+=state.position[product]
        ...
        if self.burnt_in:
            ...
            if self.zscore >= self.zscore_high and not self.short: # 1
                ...
                if pina_order_size > 0:
                    ...
                    print("BUY", bid_product, str(np.round(hedge_ratio*pina_order_size)) + "x", best_bid_coconut+1)
                    orders_coconut.append(Order(bid_product, best_bid_coconut+1, np.round(hedge_ratio*pina_order_size)))
                    print("SELL", ask_product, str(pina_order_size) + "x", best_ask_pina-1)
                    orders_pina.append(Order(ask_product, best_ask_pina-1, -pina_order_size))
            ...
            # exit signal for long trades
                ...
                if volume > 0:
                    ...
                    print("Exit SELL", product, str(volume) + "x", best_ask_coconut-1)
                    orders_coconut.append(Order(product, best_ask_coconut-1, -volume))
                elif volume < 0:
                    # buy out all existing positions
                    print("Exit BUY", product, str(-volume) + "x", best_bid_coconut+1)
                    orders_coconut.append(Order(product, best_bid_coconut+1, -volume))

                product = "PINA_COLADAS"
                volume = self.position["PINA_COLADAS"]
                if volume > 0:
                    # sell all existing positions
                    print("Exit SELL", product, str(volume) + "x", best_ask_pina-1)
                    orders_pina.append(Order(product, best_ask_pina-1, -volume))
                elif volume < 0:
                    # buy out all existing positions
                    print("Exit BUY", product, str(-volume) + "x", best_bid_pina+1)
                    orders_pina.append(Order(product, best_bid_pina+1, -volume))
            ...
            # exit signal for short trades
                ...
                if volume > 0:
                    # sell all existing positions
                    print("Exit SELL", product, str(volume) + "x", best_ask_pina-2)
                    orders_pina.append(Order(product, best_ask_pina-2, -volume))
                elif volume < 0:
                    # buy out all existing positions
                    print("Exit BUY", product, str(-volume) + "x", best_bid_pina+1)
                    orders_pina.append(Order(product, best_bid_pina+1, -volume))
