...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if mid_coconut is not None and mid_pina is not None:
            ...
            # longing the spread means we buy pina and sell coco
            synth_ask = best_ask_pina - hedge_ratio * best_bid_coconut
            # shorting the spread means we sell pina and buy coco
            synth_bid = best_bid_pina - hedge_ratio * best_ask_coconut
        ...
        if len(assets['COCONUTS'].residual) >= period:
            ...
            cocoMA = np.mean(coco_mids)
            coco_std = np.std(coco_mids)
            pina_mids = (np.array(assets['PINA_COLADAS'].ask_prices[-period:])+np.array(assets['PINA_COLADAS'].bid_prices[-period:]))/2
            pinaMA = np.mean(pina_mids)
            pina_std = np.std(pina_mids)
            delta = res-self.resMA
        ...
        if data_ready:
            ...
            if (mid_coconut >= cocoMA+factor*coco_std) and (mid_pina >= pinaMA+factor*pina_std): # 1
            ...
            elif (mid_coconut <= cocoMA-factor*coco_std) and (mid_pina <= pinaMA-factor*pina_std): #
            ...
            elif synth_bid >= self.resMA+gap: # 1
